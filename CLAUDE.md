...
### UI/UX
- [ ] **BUG: Add Question form not clearing after submit** — After a question is successfully inserted, the form fields (question text, answer, MCQ options, topics, etc.) retain their values instead of resetting. Popping widget keys from `st.session_state` + `st.rerun()` does not work — Streamlit's internal widget state survives the pop. Moving `st.rerun()` to a `try/except/else` block (to avoid `RerunException` being caught) did not fix it either. The root issue is that Streamlit preserves widget values in its own internal state even when keys are removed from `st.session_state`. **Root cause: version mismatch** — local dev runs Streamlit 1.29.0 where `pop` + `rerun` works; Streamlit Cloud runs 1.53.1 where widget state management changed and widgets retain values internally even after keys are popped. `requirements.txt` says `>=1.29.0` so Cloud installs latest. Possible fixes to try: (1) use a form key counter (`st.form(f"add_question_form_{counter}")`) so each submit creates an entirely new form instance with fresh widgets, (2) explicitly set widget keys to default values instead of popping them, (3) use `st.session_state[key] = default` before the form renders on the next run via a flag, (4) pin `streamlit==1.29.0` as a quick workaround (not recommended long-term). See `ui/app.py` lines ~727-750.
